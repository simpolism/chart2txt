...
describe('Aspect Pattern Separation', () => {
  describe('Analysis Logic', () => {
    it('composite patterns should not include single-chart patterns', () => {
      const alice: ChartData = {
        name: 'Alice',
        planets: [{ name: 'Mercury', degree: 25 }, { name: 'Mars', degree: 207 }, { name: 'Saturn', degree: 117 }],
      };
      const jake: ChartData = {
        name: 'Jake',
        planets: [{ name: 'Sun', degree: 0 }, { name: 'Moon', degree: 60 }, { name: 'Venus', degree: 120 }],
      };

      const report = formatChartToJson([jake, alice], { includeAspectPatterns: true });

      // Alice's individual analysis should have the T-Square
      const aliceAnalysis = report.chartAnalyses.find(c => c.chart.name === 'Alice');
      expect(aliceAnalysis?.patterns.some(p => p.type === 'T-Square')).toBe(true);
  ...
  describe('Integration Test', () => {
    it('should correctly separate individual and composite patterns in text output', () => {
      ...
      const aliceSection = result.substring(result.indexOf('[CHART: Alice]'), result.indexOf('[SYNASTRY: Jake-Alice]'));
