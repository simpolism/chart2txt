import { chart2txt, formatChartToJson, ChartData, AspectPattern, TSquare } from '../src/index';
...
describe('MultiChart Aspect Pattern Detection', () => {
  describe('Analysis Logic', () => {
    it('should detect T-Square patterns in synastry', () => {
      ...
      const report = formatChartToJson([chart1, chart2], { includeAspectPatterns: true });
    ...
    it('should detect Grand Trine patterns across three charts', () => {
      const chart1: ChartData = { name: 'Person A', planets: [{ name: 'Sun', degree: 0 }] };
      const chart2: ChartData = { name: 'Person B', planets: [{ name: 'Moon', degree: 120 }] };
      const chart3: ChartData = { name: 'Person C', planets: [{ name: 'Jupiter', degree: 240 }] };

      const report = formatChartToJson([chart1, chart2, chart3], { includeAspectPatterns: true });
    ...
    it('should detect T-Square patterns with transits', () => {
      ...
      const report = formatChartToJson([natalChart, transitChart], { includeAspectPatterns: true });
  ...
  describe('Integration Tests', () => {
    ...
    it('should format a global Grand Trine correctly', () => {
      const chart1: ChartData = { name: 'Person A', planets: [{ name: 'Sun', degree: 0 }], houseCusps: [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330] };
      const chart2: ChartData = { name: 'Person B', planets: [{ name: 'Moon', degree: 120 }], houseCusps: [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330] };
      const chart3: ChartData = { name: 'Person C', planets: [{ name: 'Jupiter', degree: 240 }], houseCusps: [0, 30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330] };
