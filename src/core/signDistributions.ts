...
export function calculateSignDistributions(planets: Point[], ascendant?: number) {
  const points = [...planets];
  if (ascendant !== undefined) {
    points.push({ name: 'Ascendant', degree: ascendant });
  }

  const elements: { [key: string]: string[] } = { Fire: [], Earth: [], Air: [], Water: [] };
...
export function formatElementDistribution(elements: { [key: string]: string[] }): string[] {
...
export function formatModalityDistribution(modalities: { [key: string]: number }): string[] {
  return Object.entries(modalities).map(([modality, count]) => `${modality}: ${count}`);
}

export function formatPolarityDistribution(polarities: { [key:string]: number }): string[] {
