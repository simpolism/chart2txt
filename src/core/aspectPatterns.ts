import { Point, Aspect, AspectData, AspectPattern, PlanetPosition, TSquare, GrandTrine, Stellium, GrandCross, Yod, MysticRectangle, Kite } from '../types';
 ...
 */
function calculateOrb(planet1: Point, planet2: Point, aspectAngle: number): number {
 ...
 */
function getHouseForPoint(pointDegree: number, houseCusps: number[]): number | undefined {
 ...
 */
function pointToPlanetPosition(point: Point, houseCusps?: number[]): PlanetPosition {
  const sign = getDegreeSign(point.degree);
  const house = houseCusps ? getHouseForPoint(point.degree, houseCusps) : undefined;
 ...
 */
function hasAspect(planet1: Point, planet2: Point, aspectAngle: number, orb: number): boolean {
 ...
 */
function detectTSquares(planets: Point[], houseCusps?: number[], maxOrb: number = 8): TSquare[] {
 ...
 */
function detectGrandTrines(planets: Point[], houseCusps?: number[], maxOrb: number = 8): GrandTrine[] {
 ...
 */
function detectStelliums(planets: Point[], houseCusps?: number[], minPlanets: number = 3): Stellium[] {
  ...
  signGroups.forEach((planetsInSign, sign) => {
    if (planetsInSign.length >= minPlanets) {
      ...
      const houses = planetPositions.map(p => p.house).filter(h => h !== undefined) as number[];
  ...
  if (houseCusps) {
    ...
    houseGroups.forEach((planetsInHouse, house) => {
      if (planetsInHouse.length >= minPlanets) {
        ...
        const existingSignStellium = patterns.find(p => 
          p.type === 'Stellium' && 
 ...
 */
function detectGrandCrosses(planets: Point[], houseCusps?: number[], maxOrb: number = 8): GrandCross[] {
    ...
    for (let j = i + 1; j < planets.length; j++) {
      for (let k = j + 1; k < planets.length; k++) {
        for (let l = k + 1; l < planets.length; l++) {
          ...
          const pairs = [[i, j], [k, l]];
          const otherPairs = [[i, k], [j, l], [i, l], [j, k]];
 ...
 */
function detectYods(planets: Point[], houseCusps?: number[], maxOrb: number = 5): Yod[] {
  const patterns: Yod[] = [];
 ...
 */
function detectMysticRectangles(planets: Point[], houseCusps?: number[], maxOrb: number = 8): MysticRectangle[] {
    ...
    for (let j = i + 1; j < planets.length; j++) {
      for (let k = j + 1; k < planets.length; k++) {
        for (let l = k + 1; l < planets.length; l++) {
          ...
          const combinations = [
            { oppositions: [[i, j], [k, l]], sextiles: [[i, k], [i, l], [j, k], [j, l]] },
            { oppositions: [[i, k], [j, l]], sextiles: [[i, j], [i, l], [k, j], [k, l]] },
            { oppositions: [[i, l], [j, k]], sextiles: [[i, j], [i, k], [l, j], [l, k]] }
          ];
          ...
          for (const combo of combinations) {
            ...
            if (validOppositions === 2 && validSextiles === 4) {
              ...
              patterns.push({
                ...
                oppositions: [[pos1, pos2], [pos3, pos4]],
                averageOrb
              });
            }
          }
 ...
 */
function detectKites(planets: Point[], houseCusps?: number[], maxOrb: number = 8): Kite[] {
  const patterns: Kite[] = [];
    ...
    grandTrine.planets.forEach(trinePoint => {
      planets.forEach(planet => {
        ...
        if (!isPartOfTrine) {
          ...
          if (trinePointOriginal && hasAspect(trinePointOriginal, planet, 180, maxOrb)) {
 ...
 */
export function detectAspectPatterns(planets: Point[], houseCusps?: number[]): AspectPattern[] {
